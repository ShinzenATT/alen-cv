\begin{minipage}[t]{0.4\textwidth}
    \vspace{0pt} % Trick for alignment
% here the fancy font can be taken out by removing \LobsterTwo
    {\par\centering\huge{Alen Mukaca}} \\[0.3cm]
%\faBirthdayCake~ 2001 (22) \\
%\faGlobe~ Nationalitet: Svensk \\

    {\small
    \faGraduationCap~ \underline{Titel:} Mjukvaruingenjör \\
    \faCommentsO~\underline{Språk:} \emph{Svenska} (Modersmål C2), \\ \emph{Engelska} (Flytande C1), \\ \emph{Bosniska} (Grundläggande A2).}
\end{minipage}\hfill
\begin{minipage}[t]{0.55\textwidth}
    \vspace{0pt} % Trick for alignment
    \href{tel:+46762160103}{\faPhone~ (+46) 762 160103} \\
    \href{mailto:alen@shinzle.net}{\faEnvelopeO~ \protect alen@shinzle.net} \\[0.1cm]

%\faMapMarker~ Gibraltargatan 80, 412 79 Göteborg \\
%\aiAcademiaSquare
    \href{https://linkedin.com/in/alen-mukaca/}{\faLinkedin~ \protect linkedin.com/in/alen-mukaca} \\
    \href{https://github.com/ShinzenATT}{\faGithub~ \protect github.com/ShinzenATT} \\
    %\href{https://shinzle.net}{\faGlobe~ \protect www.shinzle.net}
\end{minipage}
